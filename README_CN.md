...
2. **自动适配视频长度。** 输入完整视频后自动按 24 fps 统计唯一帧数，并按 H3 的 `17k+5` 长度网格规划片段。
3. **按段流式读取，降低系统内存峰值。** 全局只读取视频元数据与音频；每轮只解码当前约 5 秒的源画面，不再把整段长视频展开成一个 float32 图片张量。576x1024、24 fps、20 步、seed 和 22 帧 Motion Context 均保持不变。
...
9. **避免错误加速配置。** 当前 pruned INT8 基础模型下，Turbo LoRA 在稳定版中明确断开，防止把不兼容路线误当成可用加速。
...
流式版使用新的 `h3_native_loop_streamed_*` 运行名，不会自动续用旧版整片解码工作流的检查点。
